<?php /*a:1:{s:52:"/www/wwwroot/www.biand.xyz/view/index/out_order.html";i:1736870392;}*/ ?>
<!DOCTYPE html>
                ...
                if (list.length > 0) {
                    $.each(list, function (index, item) {
                        ...
                        if (item.confirm_status == 1) {
                            let confirm_status = `<div class="btn1 flexJA primary">待确认</div>`;
                            operate = `
                            ...
                            // 计算倒计时时间（假设倒计时为 10 分钟）
                            const currentTime = new Date().getTime();
                            const expirationTime = new Date(item.expiration_time).getTime(); // 从服务器获取的过期时间
                            const remainingTime = Math.max(expirationTime - currentTime, 0);
                            // 添加倒计时元素
                            let countdown = `<div class="countdown" data-order-id="${item.id}">${formatTime(remainingTime)}</div>`;
                            // 启动倒计时
                            setTimeout(() => {
                                received(item.id, 2);
                            }, remainingTime);
                        } else if (item.confirm_status == 3) {
                            let confirm_status = `<div class="btn1 flexJA danger">已取消</div>`;
                            let countdown = '';
                        }
                        html += `
                        <div class="property">
